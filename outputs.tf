...
output "dns_name" {
  ...
  value       = "${element(concat(aws_lb.main.*.dns_name, aws_lb.main_with_access_logs.*.dns_name),0)}"
}

output "zone_id" {
  description = "The canonical hosted zone ID of the load balancer (to be used in a Route 53 Alias record)."
  value       = "${element(concat(aws_lb.main.*.zone_id, aws_lb.main_with_access_logs.*.zone_id), 0)}"
}
